...
class CrossEntropyLogit(Loss):
    ...
    def forward(self, y: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        log_softmax_y = torch.log_softmax(self._prediction, dim=1)
        
        # Compute the cross-entropy loss
        loss = -torch.sum(self._target * log_softmax_y) / y.size(0)
    ...
    def backward(self) -> torch.Tensor:
        ...
        softmax_y = torch.softmax(self._prediction, dim=1)
        
        # Compute the gradient of the loss with respect to the predictions
        grad = (softmax_y - self._target) / self._prediction.size(0)
